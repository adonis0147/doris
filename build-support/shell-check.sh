...
get_md5() {
    ...
    if [[ "${tool}" == 'shellcheck' ]]; then
        if [[ "${os}" == 'linux' ]]; then
            if [[ "${arch}" == 'x86_64' ]]; then
                echo '86ee889b1e771bc8292a7043df4b962a'
            elif [[ "${arch}" == 'arm64' || "${arch}" == 'aarch64' ]]; then
                echo 'a0338c733d1283a51777b27edf9ccc96'
            fi
        elif [[ "${os}" == 'darwin' ]]; then
            if [[ "${arch}" == 'x86_64' ]]; then
                echo 'e744840256a77a1a277e81a3032f7bf4'
            fi
        fi
    elif [[ "${tool}" == 'shfmt' ]]; then
        if [[ "${os}" == 'linux' ]]; then
            if [[ "${arch}" == 'x86_64' ]]; then
                echo '1d234f204e249bf1c524015ce842b117'
            elif [[ "${arch}" == 'arm64' || "${arch}" == 'aarch64' ]]; then
                echo '5c8494fd257d5cd9db1eef48af967e47'
            fi
        elif [[ "${os}" == 'darwin' ]]; then
            if [[ "${arch}" == 'x86_64' ]]; then
                echo '42f14325207a47f3177c96683c5085a4'
            elif [[ "${arch}" == 'arm64' || "${arch}" == 'aarch64' ]]; then
                echo 'a9c51e5d4aeebfa2c8cc70af3006bc4e'
            fi
...
install_tools_if_neccessary() {
    while ! SHELLCHECK="$(check_tool 'shellcheck')"; do
        log_warning 'shellcheck was not found.'
        install_shellcheck
    done
    log_info "shellcheck found: ${SHELLCHECK}"

    while ! SHFMT="$(check_tool 'shfmt')"; do
